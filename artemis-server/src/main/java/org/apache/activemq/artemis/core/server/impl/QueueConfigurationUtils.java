...
public class QueueConfigurationUtils {
   ...
   public static void applyQueueDefaults(QueueConfiguration config, AddressSettings as) {
      applyStaticQueueDefaults(config);
      applyDynamicQueueDefaults(config, as);
   }
   ...
   public static void applyStaticQueueDefaults(QueueConfiguration config) {
      config.setTransient(config.isTransient() == null ? false : config.isTransient());
      config.setTemporary(config.isTemporary() == null ? false : config.isTemporary());
      config.setDurable(config.isDurable() == null ? true : config.isDurable());
      config.setAutoCreated(config.isAutoCreated() == null ? false : config.isAutoCreated());
      config.setInternal(config.isInternal() == null ? false : config.isInternal());
      config.setConfigurationManaged(config.isConfigurationManaged() == null ? false : config.isConfigurationManaged());
   }
